...
class MongoCourseProfileAdapter implements CourseProfilePort {
    ...
    MongoCourseProfileAdapter(UserProfileRepository repository) { this.repository = repository; }
    public Optional<UserProfile> findByUserId(String userId) { return repository.findByUserId(userId); }
    public List<UserProfile> findAllByUserIdIn(Collection<String> userIds) { return repository.findAllByUserIdIn(userIds); }
}
