...
public class ExploreService {
    ...
    public ExploreService(CourseDiscoveryRepository repository) { this.repository = repository; }
    ...
    private Pageable pageable(int page, int size) { if (page < 0 || size < 1 || size > 100) throw new IllegalArgumentException("page must be non-negative and size must be between 1 and 100"); return PageRequest.of(page, size); }
    private CourseDiscoveryResponse response(CourseDiscovery value) { CourseDiscoveryResponse r = new CourseDiscoveryResponse(); r.setCourseId(value.getCourseId()); r.setTitle(value.getTitle()); r.setSubject(value.getSubject()); r.setTags(value.getTags()); r.setEnrollmentCount(value.getEnrollmentCount()); r.setPopularityScore(value.getPopularityScore()); r.setLastActivityAt(value.getLastActivityAt()); return r; }
}
